...
export default function PerfilPage() {
  ...
  const [profile, setProfile] = useState({
    firstName: "Carlos", lastName: "Comprador",
    email: "comprador@demo.com", phone: "+57 300 000 0000",
    country: "Colombia", city: "Bogotá", bio: "",
  });
  ...
  return (
    <div className="min-h-screen bg-[var(--color-background-alt)]">
      <div className="container-max py-8 max-w-4xl">
        ...
        {/* Header card */}
        <div className="bg-white border border-[var(--color-border-subtle)] rounded-2xl p-6 mb-6 flex items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-bold text-[var(--color-on-surface)]">{profile.firstName} {profile.lastName}</h1>
        ...
        {tab === "Seguridad" && (
          <div className="space-y-4">
            <div className="bg-white border border-[var(--color-border-subtle)] rounded-2xl p-6 space-y-5">
              <h2 className="font-semibold text-[var(--color-on-surface)] flex items-center gap-2">
                <Lock size={16} className="text-[var(--color-primary)]" /> Cambiar contraseña
              </h2>
              <div className="space-y-4 max-w-sm">
                <Field label="Contraseña actual" icon={<Lock size={14} />}>
                  <div className="relative">
                    ...
                    />
                    <button onClick={() => setShowOldPass(!showOldPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--color-on-surface-variant)]">
                ...
                <Field label="Nueva contraseña" icon={<Lock size={14} />}>
                  <div className="relative">
                    <input
                      ...
                      value={passwords.nuevo}
                      onChange={e => setPasswords(p => ({ ...p, nuevo: e.target.value }))}
                      placeholder="Mínimo 8 caracteres"
                      className="input-field pr-10"
                    />
                    <button onClick={() => setShowNewPass(!showNewPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--color-on-surface-variant)]">
              ...
              <button
                onClick={handleSave}
                disabled={!passwords.old || passwords.nuevo.length < 8 || passwords.nuevo !== passwords.confirm}
                className="flex items-center gap-2 bg-[var(--color-primary)] text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-opacity"
              >
            ...
            <div className="bg-white border border-[var(--color-border-subtle)] rounded-2xl p-6 space-y-4">
              <h2 className="font-semibold text-[var(--color-on-surface)] flex items-center gap-2">
                <Shield size={16} className="text-[var(--color-primary)]" /> Sesiones activas
              </h2>
              {[
                ...
                <div key={i} className="flex items-center justify-between gap-3 py-3 border-b border-[var(--color-border-subtle)] last:border-0">
                  <div>
                    <p className="text-sm font-medium flex items-center gap-2">
                      {s.device}
                      {s.current && <span className="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium">Esta sesión</span>}
                    </p>
                    <p className="text-xs text-[var(--color-on-surface-variant)] mt-0.5">{s.location} · {s.time}</p>
                  </div>
                  {!s.current && (
                    <button className="text-xs text-red-500 hover:underline font-medium">Cerrar sesión</button>
                  )}
                </div>
              ))}
            </div>
        ...
        {tab === "Notificaciones" && (
          <div className="bg-white border border-[var(--color-border-subtle)] rounded-2xl p-6 space-y-6">
            <h2 className="font-semibold text-[var(--color-on-surface)] flex items-center gap-2">
              <Bell size={16} className="text-[var(--color-primary)]" /> Preferencias de notificaciones
            </h2>
                    ...
                    <div key={key} className="flex items-center justify-between gap-4">
                      ...
                      <button
                        ...
                        className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${
                          notifPrefs[key as keyof typeof notifPrefs] ? "bg-[var(--color-primary)]" : "bg-gray-200"
                        }`}
                      >
                        <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow-sm transition-transform ${
                          notifPrefs[key as keyof typeof notifPrefs] ? "translate-x-5" : "translate-x-0.5"
                        }`} />
                      </button>
                    </div>
                  ))}
        ...
        {tab === "Empresa" && (
          <div className="bg-white border border-[var(--color-border-subtle)] rounded-2xl p-6 space-y-5">
            <h2 className="font-semibold text-[var(--color-on-surface)]">Información de empresa</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
              ...
              <Field label="Dirección fiscal">
                <input placeholder="Calle, número, barrio..." className="input-field" />
              </Field>
              <Field label="Ciudad / Municipio">
                <input placeholder="Ciudad" className="input-field" />
              </Field>
            </div>
            <SaveBar saved={saved} onSave={handleSave} />
          </div>
...
function Field({ label, icon, children }: { label: string; icon?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="space-y-1.5">
      <label className="text-xs font-medium text-[var(--color-on-surface-variant)] flex items-center gap-1.5">
        {icon} {label}
      </label>
      {children}
    </div>
...
function SaveBar({ saved, onSave }: { saved: boolean; onSave: () => void }) {
  return (
    <div className="flex items-center justify-between pt-4 border-t border-[var(--color-border-subtle)]">
      ...
      <button
        onClick={onSave}
        className="flex items-center gap-2 bg-[var(--color-primary)] text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity"
      >
